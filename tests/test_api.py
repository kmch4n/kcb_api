...
class APITester:
    ...
    def test_search_without_auth(self):
        ...
        data = {"from_stop": "京都駅前", "to_stop": "四条河原町"}
    ...
    def test_search_with_auth(self):
        ...
        response, elapsed = self.make_request(
    ...
    def test_search_with_time(self):
        ...
        self.print_section(
            "Test 4: Search with Specific Time (堀川下長者町 → 京都駅前, 14:30)"
        )
    ...
    def test_invalid_stop(self):
        ...
        url = f"{self.base_url}/kcb_api/search"
        headers = {"Content-Type": "application/json", "X-API-Key": self.api_key}
        data = {"from_stop": "NonExistentStop", "to_stop": "京都駅前"}
...
def main():
    ...
    print(
        f"⏱️  Average Response Time: {tester.total_time / (tester.passed + tester.failed):.2f}ms"
        if (tester.passed + tester.failed) > 0
        else "N/A"
    )
